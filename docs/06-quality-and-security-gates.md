...
|---|---|---|---|
| Code quality (coverage, duplication, complexity) | SonarQube Cloud | **Yes** | GitHub Actions step; posts results to the PR; free tier to start |
...
- **Blocking:** The quality gate status check _must_ pass before merge. Repos start with the default `Sonar way` quality gate; per-repo tightening (e.g. coverage thresholds) can follow once a baseline exists.
...
- **Rationale:** Native to GitHub, zero maintenance, and free for public repos. Push protection rejects the offending push itself, which is strictly better than detecting a secret after it has landed in history.
